...
class DB:
    ...
    def get_bunker(self,
                   id_ingridient):  # Получить номер банки по Id продукта(Зерно =1, Добавки=2, Комбикорм=3, Масло=4)
        self.cursor = self.db.cursor()
    ...
    def get_product(self, id_banka):
        self.cursor = self.db.cursor()
        self.sql = """SELECT nProduct,Name FROM `kormoceh4`.`product` a, `prod_bunker` b  WHERE a.`nRec` = b.nProduct and  b.nBunker= """ + str(
            id_banka)
        self.cursor.execute(self.sql)
...
class Dialog_zerno(QDialog, Ui_Dialog_Zerno):
    def __init__(self, dbclient):
        ...
        # self.setindex(self.comboboxlist[i],zerno_[i][1])
        self.bankalist = range(11)
        self.bankalist.insert(1, self.ui.banka1)
        self.bankalist.insert(2, self.ui.banka2)
        self.bankalist.insert(3, self.ui.banka3)
        self.bankalist.insert(4, self.ui.banka4)
        self.bankalist.insert(5, self.ui.banka5)
        self.bankalist.insert(6, self.ui.banka6)
        self.bankalist.insert(7, self.ui.banka7)
        self.bankalist.insert(8, self.ui.banka8)
        self.bankalist.insert(9, self.ui.banka9)
        self.bankalist.insert(10, self.ui.banka10)

        for i in range(1, 11):
            self.bankalist[i].setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
            self.bankalist[i].customContextMenuRequested.connect(lambda: self.contextmenubanka(i))
        self.ui.banka1.customContextMenuRequested.connect(self.contextmenubanka1)

    def contextmenubanka1(self, position):
        menu = QMenu()
        ActionChange = menu.addAction(u"Изменить")

        action = menu.exec_(self.ui.banka1.mapToGlobal(position))

        if action == ActionChange:
            print "GO"
            print position

    def contextmenubanka(self, index):
        print index
        menu = QMenu()
        ActionChange = menu.addAction(u"Изменить")
        position = self.bankalist[index].pos()
        action = menu.exec_(self.bankalist[index].mapToGlobal(position))

        if action == ActionChange:
            print "GO"
            print position

    def setindex(self, combo, text):
...
def cmd_ves12start():
    ...
    print ("COMMAND_VES12_START")
    logging.info("==!COMMAND_VES12_START")
...
def cmd_ves3start():
    ...
    print ("COMMAND_VES3_START")
    logging.info("==!COMMAND_VES3_START")
...
def cmd_ves2unload():
    ...
    print ("COMMAND_VES2_UNLOAD")
    logging.info("==!COMMAND_VES2_UNLOAD")
...
def cmd_ves3unload():
    ...
    print ("COMMAND_VES3_UNLOAD")
    logging.info("==!COMMAND_VES3_UNLOAD")
...
def cmd_oilstart():
    ...
    print ("COMMAND_OIL_START")
    logging.info("==!COMMAND_OIL_START")
...
def cmd_mixerunload():
    ...
    print ("COMMAND_MIXER_UNLOAD")
    logging.info("==!COMMAND_MIXER_UNLOAD")
...
class ThreadSendCmd(QtCore.QObject):
    ...
    @QtCore.pyqtSlot()
    def process(self):
        print "+thread ThreadSendCmd started  cmd= " + str(self.cmd)
        plcglobal.waitret()
...
class ThreadWorkZames(QtCore.QObject):
    ...
    def process(self):
        ...
        while self._isRunning is True:
            ...
            # self.starttimer.emit(300000)
            # while(plcglobal.countqueue != 0):
            #     noriareadyinit=0
            #     dbstatus = mbclient.read(REG_STATUS)
            #     noriareadyinit=getbit(dbstatus,NORIAREADYLOAD)
            #     print "= Waiting noriareadyinit"
            #     while(noriareadyinit!=1):
            #         pass
            #     if noriareadyinit == 1:
            ...
            #             plcglobal.send_cmd(COMMAND_VES3_START)
            #
            #         ves1readyunload = getbit(dbstatus,VES1READYLOAD)
            #         ves2readyunload = getbit(dbstatus,VES2READYLOAD)
            #         ves3readyunload = getbit(dbstatus,VES3READYLOAD)
            #         mixerreadytoload = getbit(dbstatus,MIXERREADYLOAD)
            #         line1empty = getbit(dbstatus,LINE1READYLOAD)
            #         line2empty = getbit(dbstatus,LINE2READYLOAD)
            #         print "= Waiting vesreadyunload and lines_readyunload and mixerreadyload"
            #         if ves1readyunload==1 and ves2readyunload==1 and ves3readyunload==1:
            #             if line1empty==1 and line2empty==1:
            #                 if mixerreadytoload == 1:
            ...
            #         noriareadyload = getbit(dbstatus,NORIAREADYLOAD)
            #         if mixerreadyunload == 1 and noriareadyload == 1:
            ...
            # print "= ves1readyload => ves1_start_"
            # if self.session_begin==False:
            #     print "@Начало сессии"
            #     self.startsession.emit()  # Отправка рецепта в ПЛК1(весы1, весы2 и весы3)
            #     self.session_begin=True
            # while self.session_begin!=False:
            #     print "@Сессия идет!"
            #     for i in range(0,16):
            ...
            #     if mixerreadyload==1 & line2readyload==1 & ves2readyunload==1:
            #         plcglobal.waitret()
            #         plcglobal.send_cmd(COMMAND_VES2_UNLOAD)
            #         plcglobal.waitret()
            #         plcglobal.send_cmd(COMMAND_OIL_START)
            ...
            #     if ves3readyunload==1:
            #         plcglobal.waitret()
            #         plcglobal.send_cmd(COMMAND_VES3_UNLOAD)
            #     mixerreadyunload=0
            #     mixerreadyunload=getbit(dbstatus,MIXERREADYUNLOAD)
            #     if mixerreadyunload==1:
            #         plcglobal.waitret()
            #         plcglobal.send_cmd(COMMAND_MIXER_UNLOAD)
            ...
            # self.startsession.emit()  # plcglobal.recept.zakaz.pull() заполняем массив заказа
            ...
            # plcglobal.recept.indexmuchdobavka = dobavka.index(
            #     max(dobavka)) + 8  # индекс банки с максимальным значением добавки
            # print "all dobavka" + str(alldobavka)
            # print "max " + str(max(dobavka))
            #
            # if alldobavka > MAX_DOBAVKA:
            #     plcglobal.recept.two_stage = True
            # else:
            #     plcglobal.recept.two_stage = False
            # print "Отправка рецепта зерно"
            # for i in range(0, 8):
            #     print plcglobal.recept.zakaz.getelement(i)
            #     mbclient.send(i + 20, plcglobal.recept.zakaz.getelement(i))  # Отправка рецепта зерна по 27
            # if plcglobal.recept.two_stage == True:
            #     print "Отправка рецепта добавка(макс)"
            #     print  plcglobal.recept.zakaz.getelement(plcglobal.recept.indexmuchdobavka)
            #     mbclient.send(plcglobal.recept.indexmuchdobavka + 20, plcglobal.recept.zakaz.getelement(
            #         plcglobal.recept.indexmuchdobavka))  # Отправка рецепта добавки с индеком с переполненным
            # else:
            #     print "Отправка рецепта добавка(все)"
            #     for i in range(8, 14):
            #         if i != plcglobal.recept.indexmuchdobavka:
            #             print plcglobal.recept.zakaz.getelement(i)
            #             mbclient.send(i + 20,
            #                           plcglobal.recept.zakaz.getelement(i))  # Отправка рецепта добавок с 28 по 33
            #
            ...
            #     print "Достигнуто максимальное значение весов, обнулите"
            # print "Ожидание обнуления весов"
            # while ves2 > VESMAX & ves3 > VESMAX:
            #     pass
            # print "весы обнулены"
            ...
            # plcglobal.send_cmd_val(COMMAND_NORIA_INIT,plcglobal.recept.selectkormnumbanka[0])
            self.session_begin = True
            plcglobal.recept.count = 1
            print "session begin"
            while self.session_begin == True:
                # print "session begin"
                if plcglobal.recept.two_stage == True:
                    print "Отправка рецепта зерно и добавки(половинный)"
                    for i in range(8, 14):
                        if i != plcglobal.recept.indexmuchdobavka:
                            print plcglobal.recept.zakaz.getelement(i) / 2
                            mbclient.send(i + 20, plcglobal.recept.zakaz.getelement(
                                i) / 2)  # Отправка рецепта добавок с 28 по 33
                    for i in range(0, 8):
                        print plcglobal.recept.zakaz.getelement(i) / 2
                    ...
                    print "Отправка рецепта добавка и зерно(все)"
                    for i in range(8, 14):
                        print plcglobal.recept.zakaz.getelement(i)
                        mbclient.send(i + 20,
                                      plcglobal.recept.zakaz.getelement(i))  # Отправка рецепта добавок с 28 по 33
                    for i in range(0, 8):
                        ...
                        # plcglobal.recept.count=1
                        # print "while plcglobal recept count !=0"
                        # print "count = "+ str(plcglobal.recept.count)
                        # logging.info("==! count = "+str(plcglobal.recept.count))
                        # plcglobal.recept.stages.reinit()
                        # logging.info("==! zames start! ")
                        ...
                        #     delay(1000)
                        #
                        #     if ves2readyload == 1 and COMMAND_VES12_START not in plcglobal.recept.stages.list:
                        #
                        #         delay(1000)
                        #         # plcglobal.waitret()
                        #         # plcglobal.send_cmd(COMMAND_VES12_START)
                        ...
                        #         plcglobal.recept.stages.list.append(COMMAND_VES12_START)
                        #     else:
                        #         if ves2readyload == 1 and plcglobal.recept.count > 1:
                        #
                        #
                        #     # if ves3readyload == 1 and COMMAND_VES3_START not in plcglobal.recept.stages.list:
                        #     if ves3readyload == 1 and plcglobal.recept.count>1:
                        #         delay(1000)
                        #         # plcglobal.waitret()
                        #         # plcglobal.send_cmd(COMMAND_VES3_START)
                        #         print ("COMMAND_VES3_START")
                        ...
                        #         plcglobal.recept.stages.list.append(COMMAND_OIL_START)
                        #
                        #     if ves3readyunload == 1 and mixerreadyload==1 and COMMAND_VES3_UNLOAD not in plcglobal.recept.stages.list and COMMAND_VES2_UNLOAD in plcglobal.recept.stages.list:
                        #         delay(1000)
                        #         # plcglobal.waitret()
                        #         # plcglobal.send_cmd(COMMAND_VES3_UNLOAD)
                        #         print ("COMMAND_VES3_UNLOAD")
                        ...
                        #
                        #
                        #     if mixerreadyunload == 1 and plcglobal.tick==True and COMMAND_MIXER_UNLOAD not in plcglobal.recept.stages.list :
                        #         print "mixerreadyunload = "+str(mixerreadyunload)
                        ...
                        #         print "plglobal.recept.count ="+str(plcglobal.recept.count)
                        #         plcglobal.recept.stages.reinit()
                        #         if plcglobal.recept.count==0:
                        #             self.session_begin=False
                        #             print "session_begin=False"
                        # break

                        # ves2readyload=0
                        # while(ves2readyload!=1):
                        #     dbstatus=mbclient.read(REG_STATUS)
                        #     ves2readyload = getbit(dbstatus, VES2READYLOAD)
                        # if ves2readyload==1:
                        #     plcglobal.waitret()
                        ...
                        # mixerreadyload=0
                        # while mixerreadyload!=1:
                        #     dbstatus = mbclient.read(REG_STATUS)
                        #     mixerreadyload = getbit(dbstatus, MIXERREADYLOAD)
                        # line2readyload=0
                        # while line2readyload!=1:
                        #     dbstatus = mbclient.read(REG_STATUS)
                        #     line2readyload = getbit(dbstatus,LINE2READYLOAD)
                        # ves2readyunload=0
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, dbclient):
        ...
        self.movies.insert(78, objs(
            obj(self.mixershnek2, "shnek_nakl_stop.gif", "", "shnek_nakl_begin.gif")))  # Шнек наклонный перед норией
    ...
    def mixershnek1PopUp(self, position):
        ...
        if action == OnAction:
            print "=PopUp mixershnek1 On"
            self.startThreadCmd(COMMAND_SHNEKMIXER_ON)
        if action == OffAction:
            print "=PopUp mixershnek1 Off"
    ...
    def ves3PopUp(self, position):
        ...
        if action == UnloadAction:
            print "=PopUp VES3 Unload"
            self.startThreadCmd(COMMAND_VES3_UNLOAD_HAND)
    ...
    def startThreadCmd(self, cmd):
        print "Try start thread ThreadCmd"
    ...
    def workzamesstartslot(self):
        print('start! slot from threadPLCZames signal_  startsession emit ')
