...
class CONFIG:
    ...
    def __init__(self):
        ...
        if self.parser.has_section("DATABASE") is False:
            ...
            self.namedb = unicode(self.parser.get('DATABASE', 'SQL_DATABASE', 'kormoceh5'), 'utf8')
...
class Dialog_dozakaz(QDialog,Ui_Dialog_dozakaz):
    ...
    def itemclicked(self):
        ...
        list_=[]
        list_.append(unicode(item0.text()))
        list_.append(unicode(item1.text()))
    ...
    def load_this(self,val):
        ...
        for i in range(0,len(self.bunkers)):
            if self.bunkers[i].ready():
                ...
                self.bunkers[i].send_val(val*10)
                return True
            else:
...
class PLC(object):
    ...
    def pull_prodbunker(self):
        list_ = []
        for _id,prod in self.products.iteritems():
    ...
    def process(self):
        ...
        while (self._isRunning is True) :
            ...
            while plcglobal.session.session_begin == True:
                ...
                if ves3readyunload == 1 and plcglobal.tick == True  and not plcglobal.commands[COMMAND_VES3_UNLOAD].is_registered():
                    plcglobal.tick = False
                    time.sleep(0.01)
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, dbclient):
        ...
        for id,qobject in self.warnings.iteritems():
            qobject.hide()
