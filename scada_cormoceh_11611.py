...
def writedebug(msg):
    try:
        print str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + u": " + msg
        logging.debug(msg)
    except Exception as err:
        print  str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+" "+str(err)
        logging.error(traceback.format_exc())
def write(msg):
    try:
        print str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+u": "+msg
        plcglobal.textout.append(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+u": "+msg)
        logging.debug(msg)
    except Exception as err:
        print  str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))+" "+str(err)
        logging.error(traceback.format_exc())
...
class commands(object):
    ...
    def execute(self,timeout):
        ...
        # if self.name not in plcglobal.command_list:
            ...
            write(u"Отменена избыточная отправка команды" + self.name)
    ...
    def checkexecute(self):
        if self.run == True:
            if getbit(plcglobal.status,self.flag)==0:
                ...
                write(u"ПЛК успешно получил команду: " + self.name + u" ")
                return True
            else:
                if self.times < self.timeout:
                    ...
                    if self.attempts < self.MAXATTEMPTS:
                        ...
                        self.times = 0
                        plcglobal.send_cmd(self.cmd)
                        write(u"Повторно отправлена команда("+str(self.attempts)+u"):" + self.name)
                        ...
                        write(u"ПЛК не получил команду: " + self.name + u"- ОСТАНОВ  замешивания")
...
class PLC(object):
    def __init__(self, mbclient):
        ...
        self.errors = {ERRORWEIGHT1: "Error weight1", ERRORWEIGHT2: "Error weight2",ERRORWEIGHT3:"Error weight3",ERRORLINEK:"Error LINEK" ,ERRORPERKLAPAN:"Error PerKlapan", ERRORMIXER:"Error Mixer",ERRORLINE1:"Error Line1",ERRORLINE2:"Error Line2", ERRORAWILA:"Error Awila",
                       ERRORMIXERNOTOPEN: "Error mixer not open", ERRORMIXERNOTUNLOAD:"Error mixer not unload", ERROREXECUTECMD:"Command not execute" ,ERRORSHNEK: "Error shnek" }
        ...
        self.shnekszerno = {ISHNEK1:shnek(getweight1,regerror,50,60),ISHNEK2:shnek(getweight1,regerror,50,60),
                            ISHNEK3:shnek(getweight1,regerror,50,60),ISHNEK4:shnek(getweight1,regerror,50,5),
        ...
        self.shneksdobavka = {IFLEX1:shnek(getweight3,regerror,10,60),IFLEX2:shnek(getweight3,regerror,10,60),
                              IFLEX3: shnek(getweight3, regerror, 10,60), IFLEX4: shnek(getweight3, regerror, 10,60),
                              IFLEX5: shnek(getweight3, regerror, 10,60), IFLEX6: shnek(getweight3, regerror, 10,60)}
        ...
        while(self._isRunning == True):
            ...
            try:
                for ishnek_,shnek_ in plcglobal.shnekszerno.iteritems():
                    ...
                    shnek_.chkstate()
                    if shnek_.started:
                        write(u"Запущен шнек " + str(ishnek_))
                    if shnek_.endwork == True:
                        write(u"Шнек закончил работу" + str(ishnek_))
                        db.registerweight(plcglobal.getnumbanka(ishnek_), shnek_.weightend)
                    if shnek_.error:
                ...
                write(u" Ошибка обработки состояния шнека("+str(shneknow_)+u")" + str(err))
                logging.error(traceback.format_exc())
            error = 0
            if (plcglobal.pulerrors):
            ...
            # for i in range(0,10):
            #     if plcglobal.shnekszerno[i]==1:
            #         print "Шнек зерно"+str(i+1)+"Включен"
            #         self.lastweight =plcglobal.listplcrg16.list[VES1INDEX]
            #         while(plcglobal.shnekszerno[i]==1):
            #             pass
            #         plcglobal.weight1 = plcglobal.listplcrg16.list[VES1INDEX] - self.lastweight
            #         print "Сохранить "+str(plcglobal.weight1/10.0)
            #         print "Шнек зерно" + str(i+1) + "Выключен"
            # for i in range(0, 6):
            #     if plcglobal.shnekszerno[i] == 1:
            #         print "Шнек доб" + str(i + 1) + "Включен"
            #         while (plcglobal.shnekszerno[i] == 1):
            #             pass
            #         print "Шнек доб" + str(i + 1) + "Выключен"
    def stop(self):
        self._isRunning = False
        ...
        while self._isRunning is True:
            ...
            while self.session_begin == True:
                ...
                if self.newcicle==True :    # Если текущий счетчик замесов обнулен
                    ...
                    writedebug(u"--while plcglobal.recept.count !=0")
                    writedebug(u"--count = "+ str(plcglobal.recept.count))
                    writedebug(u"==! zames start! ")
                ...
                if plcglobal.lasterror != plcglobal.newerror:
                    ...
                    if error == (1 << ERRORPERKLAPAN):
                        ...
                        write(str(datetime.datetime.now()) + u" Ожидание устранения неисправности перекидного клапана")
                        while (lasterror == 1):
                    ...
                    if error == (1 << ERRORLINEK):
                        write(str(datetime.datetime.now()) + u" Ожидание устранения неисправности линии нории")
                        while (lasterror == 1):
                            lasterror = getbit(plcglobal.listplcrg16.list[REG_ERROR], ERRORLINEK)
                    ...
                    if error == (1<<ERRORMIXERNOTUNLOAD):
                        write(str(datetime.datetime.now()) + u" Ожидание устранения ошибки ERRORMIXERNOTUNLOAD ")
                        while (lasterror == 1):
                            lasterror = getbit(plcglobal.listplcrg16.list[REG_ERROR], ERRORMIXERNOTUNLOAD)
                    if error == (1<<ERRORMIXERNOTOPEN):
                        write(str(datetime.datetime.now()) + u" Ожидание устранения ошибки ERRORMIXERNOTOPEN")
                        while (lasterror == 1):
                ...
                if ves2readyload == 1 and  not plcglobal.commands[COMMAND_VES12_START].is_registered():
                    ...
                    if plcglobal.recept.count == plcglobal.recept.needcount:
                        write(u"СТАРТ ЗАМЕСА : " + str(plcglobal.recept.receptname))
                    ...
                    if plcglobal.recept.count<=0 and self.session_end==False:
                        ...
                        if plcglobal.countqueue > 0:  # если очередь не пуста
                            ...
                            write( u"Изготовление рецепта  " + plcglobal.recept.procinfo.receptnow + u"завершается")
                            ...
                            write(u"Изготовление рецепта  " + plcglobal.recept.procinfo.receptnow + u"завершается")
                            write(u"Очередь пуста, после изготовления рецепта, окончание работы")
                    if plcglobal.recept.count>0:
                        ...
                        write(u"Загрузка весов2 - Рецепт " + plcglobal.recept.procinfo.receptnow + u" номер замеса "+str(plcglobal.recept.procinfo.countves2))
                # if noriaready == 1:
                ...
                if ves2readyunload == 1 and line2readyload == 1 and mixerreadyload == 1 and not plcglobal.commands[COMMAND_VES2_UNLOAD].is_registered():
                    write(u"Выгрузка весов2 - Рецепт " + plcglobal.recept.procinfo.receptnow + u" номер замеса " + str(plcglobal.recept.procinfo.countves2))
                    ...
                    plcglobal.recept.stagenameline12 = plcglobal.recept.procinfo.receptnow + u" " + str(plcglobal.recept.procinfo.countline2) +u" из "+  str(plcglobal.recept.needcount)
                ...
                if ves3readyunload == 1 and plcglobal.tick == True and plcglobal.commands[COMMAND_VES12_START].is_done and not plcglobal.commands[COMMAND_VES3_UNLOAD].is_registered():
                    time.sleep(0.01)
                ...
                if mixerreadyunload == 1 and not plcglobal.commands[COMMAND_MIXER_UNLOAD].is_registered() and oilready == 1 and plcglobal.commands[COMMAND_VES3_UNLOAD].is_done and dobdobavkaready == 1:
                    ...
                    if self.session_end==True:
                        write(u"Счетчик <Количество циклов> достиг нуля- сессия завершена; countcicle = " +str(plcglobal.countqueue) )
                        self.session_begin = False
                        self._isRunning = False
                        self.newcicle = False
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def clkbtnaddstage(self):
        if  self.combostages.currentText() == u"Весы12 ЗАГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_VES12_START")
        if self.combostages.currentText() == u"Весы3 ЗАГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_VES3_START")
        if self.combostages.currentText() == u"Весы1 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_VES1_UNLOAD")
        if self.combostages.currentText() == u"Весы2 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_VES2_UNLOAD")
        if self.combostages.currentText() == u"Весы3 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_VES3_UNLOAD")
        if self.combostages.currentText() == u"МАСЛО ЗАГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_OIL_START")
        if self.combostages.currentText() == u"МИКСЕР ВЫГРУЗИТЬ":
            plcglobal.recept.stages.add("COMMAND_MIXER_UNLOAD")
    def clkbtnsubstage(self):
        if  self.combostages.currentText() == u"Весы12 ЗАГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_VES12_START")
        if self.combostages.currentText() == u"Весы3 ЗАГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_VES3_START")
        if self.combostages.currentText() == u"Весы1 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_VES1_UNLOAD")
        if self.combostages.currentText() == u"Весы2 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_VES2_UNLOAD")
        if self.combostages.currentText() == u"Весы3 ВЫГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_VES3_UNLOAD")
        if self.combostages.currentText() == u"МАСЛО ЗАГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_OIL_START")
        if self.combostages.currentText() == u"МИКСЕР ВЫГРУЗИТЬ":
            plcglobal.recept.stages.sub("COMMAND_MIXER_UNLOAD")
    def statemode(self):
    ...
    def clkunload3(self):
        writedebug(u"POPUP команда Dsгрузить весы3 ")
    ...
    def clklinek(self):
        ...
        self.startThreadCmd(COMMAND_NORIA_INIT + (plcglobal.recept.selectkormnumbanka[0] << 8))
    def clkoil(self):
        logging.debug(u"POPUP команда Загрузить масло ")
    ...
    def workzamesendslot(self):
        ...
        try:
            ...
            logging.debug(err)
        plcglobal.decqueue()
        writedebug("--count queue = "+str(plcglobal.countqueue))
        if plcglobal.countqueue==0:
            self.deleteallrow()
    ...
    def workzamesgetreceptslot(self):
        try:
            ...
            logging.debug(u"Загрузка рецепта: " + str(self.tableWidget.item(0, 0).text())+u" количесвто замесов "+str(plcglobal.recept.needcount))
            plcglobal.recept.setcount(plcglobal.recept.needcount)
            logging.debug(u"Количество замешиваний: " + str(plcglobal.recept.count ))
            plcglobal.recept.selectkormnumbanka = []
            for i in range(0, 6):
                if self.interface_[i].state is True:  # выделенную банку
                    plcglobal.recept.selectkormnumbanka.append(i + 1)  # помещаем в список банок для комбикорма
                    logging.debug(u"Выгрузка комбикорма в  банку №" + str(i))
            plcglobal.recept.getzakazcomplete = True
            write(u"Заказан рецепт: "+plcglobal.recept.procinfo.receptnow)
            write(u"Количесвто замесов: " + str(plcglobal.recept.count))
            current_recept = {}
            ...
            for i in range(0, 6):
                if str(i + 1) in (index_banka_):
                    plcglobal.recept.selectkormnumbanka.append(i)
                else:
                    pass
            ...
            plcglobal.recept.getstartslotcomplete = True
    def workerzamescheckslot(self):
        write(u"--Проверка заполнености силоса готового комбикорма, силос №"+str(plcglobal.recept.selectkormnumbanka[0]))
        ...
        if self.movies[plcglobal.kormbankauplevel[plcglobal.recept.selectkormnumbanka[0]]].state == True:
            plcglobal.recept.nullcount()
            write(u"Выбранный силос заполнен, окончание изготовдления текущего корма: " + plcglobal.recept.procinfo.receptnow)
        else:
            write(u"--Выбранный силос(№" +str(plcglobal.recept.selectkormnumbanka[0]+1) + u")не заполнен")
