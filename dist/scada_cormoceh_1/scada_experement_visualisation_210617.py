...
class MBclient(object):
    ...
    def send(self,rgadr,val):
        ...
        try:
            ...
            print self.client.last_error() + "Error write holding register with addr " + str(rgadr)
        free(self)
    def read(self,rgadr):
        try:
            self.checkconnecttcp()
...
class PLC(object):
    ...
    def waitstate(self,needstate_):
        self.needstate=needstate_
        while self.state is not self.needstate:
            self.get_state()
            delay(1000)
        print "waitstatereturn "+str(self.state)
        self.send_ret(0)
        self.get_state()
        print "waitstatereturn " + str(self.state)
...
class ThreadWorkZames(QtCore.QObject):
    ...
    def process(self):
        ...
        while  self._isRunning is True:
            ...
            print "ПЛК в статусе  <Ожидание команды> !"+str(plcglobal.state)
            self.s2.emit()
            plcglobal.send_cmd(100)
            print "Send  =Begin work= and wait answer"
            plcglobal.waitstate(1)

            print "Receive =Begin work= "+str(plcglobal.state)
            ...
            plcglobal.waitstate(1)
            print "Получен ответ от ПЛК <Приемка комбикорма перенастроена>"+str(plcglobal.state)
            while(plcglobal.recept.count is not 0):
                print "Отправлена команда на изготовление рецепта"
                plcglobal.send_cmd(1)
                print "Ожидание ответа от ПЛК"
                plcglobal.waitstate(1)
                print "ПЛК завершил завешивание, ПЛК ожидает команду"+str(plcglobal.state)
                plcglobal.recept.deccount()
            print "Отправлена команда ПЛК <Конец сессии>"
            plcglobal.send_cmd(102)
            print "Ожидание ответа от ПЛК"
            plcglobal.waitstate(1)
            print "Получен ответ от ПЛК <Конец сессии>"+str(plcglobal.state)
            print "Send =End work="
            plcglobal.send_cmd_val(103,0)
            plcglobal.waitstate(1)
            print "Получен ответ =End work="+str(plcglobal.state)
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self,dbclient,mbclient,plc_):
        ...
        self.movies.insert(63,objs( obj(self.zernoshnek9, "shnek9zernostop.png", "shnek9zernorun.png", ""),obj(self.zernoshnek9_2, "shnek_nakl_stop.gif", "", "shnek_nakl_begin.gif")))
        #reg11 PLC9 (zerno)
        self.movies.insert(64,objs( obj(self.zernoshnek8, "shnek8zernostop.png", "shnek8zernorun.png", ""),obj(self.zernoshnek8_2, "shnek_nakl_stop.gif", "", "shnek_nakl_begin.gif")))
