...
class ThreadWorkZames(QtCore.QObject):
    ...
    def process(self):
        ...
        while self._isRunning is True:
            ...
            while self.session_begin == True:
                ...
                while(plcglobal.recept.count>0):
                    ...
                    if ves3readyunload == 1 and plcglobal.tick==True and COMMAND_VES3_UNLOAD not in plcglobal.recept.stages.list and COMMAND_VES2_UNLOAD in plcglobal.recept.stages.list:
                        delay(1000)
                        plcglobal.waitret()
        ...
        print str(datetime.datetime.now())+"Счетчик <Количество замесов> достиг нуля- изготовление рецепта завершено!"
        self._isRunning = False
        self.finished.emit()
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def workerzamescheckslot(self):
        ...
        newitem = QTableWidgetItem(str(int(plcglobal.recept.needcount)-plcglobal.recept.count)+u" из "+plcglobal.recept.needcount)
