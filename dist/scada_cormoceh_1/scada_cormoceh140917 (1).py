...
def sendrecept_2():
    print "Отправка рецепта добавка и зерно(половинный)"
    for i in range(8, 14):
        ...
        while self._isRunning is True:
            ...
            while self.session_begin == True:
                ...
                if plcglobal.recept.two_stage == True:
                    plcglobal.recept.commands.append(sendrecept_2())
                    plcglobal.recept.count *= 2
                    # print "Mul ="+str(plcglobal.recept.count)
                else:
                    plcglobal.recept.commands.append(sendrecept())
                while (plcglobal.recept.count != 0):
                    plcglobal.recept.commands.append(cmd_ves12start())
                    plcglobal.recept.commands.append(cmd_ves3start())
                    plcglobal.recept.commands.append(cmd_ves2unload())
                    plcglobal.recept.commands.append(cmd_ves3unload())
                    plcglobal.recept.commands.append(cmd_oilstart())
                    plcglobal.recept.commands.append(cmd_mixerunload())
                    plcglobal.recept.count -= 1
            while (plcglobal.recept.commands):
                plcglobal.recept.commands.pop(0)()
            break
