...
from designdozakaz import Ui_Dialog
...
class Dialog_recept(QDialog, Ui_Dialog_recept):
    ...
    def itemclicked(self):
        ...
        self.nownrec_recept = str(self.dbclient.getidrecept(unicode(self.ui.listwidgetkorm.currentItem().text()))[0])
    ...
    def btnaddzernoclk(self):
        idproduct = self.dbclient.get_id_product(unicode(self.ui.combozerno.currentText()))
        # print idproduct
    ...
    def btnadddobavkaclk(self):
        idproduct = self.dbclient.get_id_product(unicode(self.ui.combodobavka.currentText()))
        # print idproduct
    ...
    def savebtnclk(self):
        ...
        print self.receptlist
        self.dbclient.changerecept(self.nownrec_recept, self.receptlist)
class Dialog_dozakaz(QDialog,Ui_Dialog):
    def __init__(self, dbclient):
        # global recept_ini
        QDialog.__init__(self)
        self.ui = Ui_Dialog_recept()
    ...
    def pull_table_zakazlist(self):
        self.zakazlist = self.dbclient.get_listdozakaz()
        # for nRec_ in self.zakazlist:
        #     name_ = str(nRec_)+' '+self.zakazlist[nRec_]
        #     self.ui.listwidgetkorm.addItem(name_)
    ...
    def pull_tablewidget_zakaz(self):
        self.ui.tablerecept.setRowCount(0)
        # for item in self.receptlist:
        #     print str(item[0])+u" "+str(item[1])
        #     nameprod = (self.dbclient.get_productname(item[0]))
...
class Dialog_zerno(QDialog, Ui_Dialog_Zerno):
    ...
    def btnapplyclk(self):
        ...
        for i in range(1,11):
            ...
            if nameprod != 'Not active':
                if unicode(self.zerno_[i][1]) != nameprod:
...
def loadrecepts():
    ...
    if LOADRECEPTFROMINI == True:
        ...
        for nrecrecept in recepts:
            receptlist_[nrecrecept] = unicode(db.get_productname(recepts[nrecrecept])[0][0])
        print receptlist_
        return receptlist_
                ...
                for ishnek_,shnek_ in plcglobal.shnekszerno.iteritems():
                    ...
                    if shnek_.endwork == True:
                        ...
                        try:
                            ...
                            db.registerweight(plcglobal.getnumbanka(ishnek_), shnek_.weightend - shnek_.weightbegin,plcglobal.recept.nreczakaz)
                        except Exception as err:
            ...
            while self.session_begin == True:
                ...
                if plcglobal.lasterror != plcglobal.newerror:
                    ...
                    if error == (1 << ERRORMIXER):
                        # write(u"Прерывание изготовления рецепта в связи с ошибкой миксера(возможно выключено тепловое реле)")
                        # self.session_begin = False
                        # self._isRunning = False
                        # dbstatus = 0
                        write(u" Ожидание устранения ошибки миксера ")
                        while (lasterror == 1):
                            lasterror = getbit(plcglobal.listplcrg16.list[REG_ERROR], ERRORMIXER)
                    if error == (1 << ERRORLINE1):
                        write(u"Прерывание изготовления рецепта в связи с ошибкой линии весов1(возможно выключено тепловое реле)")
                        self.session_begin = False
                        self._isRunning = False
                        dbstatus =0
                ...
                if ves2readyload == 1 and  not plcglobal.commands[COMMAND_VES12_START].is_registered():
                    ...
                    if plcglobal.recept.count>0:
                        ...
                        plcglobal.commands[COMMAND_VES12_START].register()
                ...
                if mixerreadyunload == 1 and not plcglobal.commands[COMMAND_MIXER_UNLOAD].is_registered() and oilready == 1 and plcglobal.commands[COMMAND_VES3_UNLOAD].is_registered() and dobdobavkaready == 1 and line2readyload == 1:
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def workzamesgetreceptslot(self):
        def getnrecrecept():
            for nrec, name in plcglobal.recept.receptlist.iteritems():
                if plcglobal.recept.receptlist[nrec] == plcglobal.recept.receptname:
                    print nrec
                    return nrec
        ...
        if LOADRECEPTFROMINI:
            ...
            plcglobal.recept.receptname = unicode(self.tableWidget.item(0, 0).text())
            plcglobal.recept.needcount = plcglobal.needcountlist.pop(0)
            plcglobal.recept.procinfo.receptnow = (plcglobal.receptnamelist.pop(0))
            if plcglobal.recept.procinfo.receptprev == '':
                plcglobal.recept.procinfo.receptprev = plcglobal.recept.procinfo.receptnow
            ...
            ves1zames = int(self.tableWidget.item(0, 2).text())/1.0
            plcglobal.recept.koef = ves1zames /1000
            dict_ = {}
            ...
            plcglobal.recept.zakaz.pull(list_)
            # print plcglobal.recept.zakaz.list
            #3. Получить nrec zakaz
            plcglobal.recept.nreczakaz =  db.createzakaz(plcglobal.recept.nrecrecept,ves1zames)
            #4. Записать значения в zakaz_det по nrec zakaz
            for nrecbunker,val in dict_.iteritems():
                try:
                    db.register_zakaz(plcglobal.recept.nrecrecept,nrecbunker,plcglobal.recept.nrecrecept,plcglobal.recept.koef)
                except Exception as err:
                    writedebug(u"Ошибка регистрации заказа в БД: "+str(err))
            #5. Выбор банки комбикорма
            list_= db.get_bunkerrecept(plcglobal.recept.nrecrecept)
            plcglobal.recept.selectkormnumbanka = []
            if list_:
                plcglobal.recept.selectkormnumbanka = list_
