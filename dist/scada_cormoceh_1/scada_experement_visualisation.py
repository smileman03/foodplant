...
class ThreadWorkZames(QtCore.QObject):
    ...
    def process(self):
        ...
        while  self._isRunning is True:
            ...
            # i=0
            # print "Проверка статуса выбранного силоса "+str(plcglobal.recept.selectkormnumbanka[i])
            # if(listplccoils=listsilosdobavka[plcglobal.recept.selectkormnumbanka[i]])
            ...
            while(plcglobal.recept.count is not 0):
                ...
                print "Ожидание ответа от ПЛК"
                plcglobal.waitret()
                print "ПЛК завершил завешивание"
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self,dbclient,mbclient,plc_):
        ...
        self.addtoqueuebtn.clicked.connect(self.addtoqueuebtnclk)
        self.mbclient=mbclient
        self.plc=plc_
       ...
       # self.movies.insert(101, obj(self.autoruchnokombikorm, "statusfalse.png","statustrue.png", ""))
        ...
        self.comboBoxRecept.addItems(self.plc.recept.receptlist.keys())
    ...
    def deleteallrow(self):
        for i in range(0,self.currentrow):
    ...
    def loadrecept(self,name):
        ...
        namerecept=name
        current_recept={}
        current_recept=self.plc.recept.receptlist.get(namerecept)
    ...
    def startbtnclk(self):
        ...
        self.plc.recept.count=int(self.tableWidget.item(0,1).text())
        self.plc.recept.selectkormnumbanka=[]
        for i in range(0, 6):
            if self.interface_[i].state is True:
                self.plc.recept.selectkormnumbanka.append(i + 1)
